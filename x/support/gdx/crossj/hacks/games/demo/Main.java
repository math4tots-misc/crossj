...
public final class Main implements ApplicationListener {
    ...
    public void render() {
        ...
        hangulFont.draw(batch, "한글 hello 是", Gdx.graphics.getWidth() / 2, Gdx.graphics.getHeight() / 2);
        hanjaFont.draw(batch, "之不一人以有了为道是子的来大也十其上二而中曰下于三得在", Gdx.graphics.getWidth() / 8,
                3 * Gdx.graphics.getHeight() / 4);
        hanjaFont.draw(batch, "年我他王说事见将者去日天州出后又自此个时无军太这与月", Gdx.graphics.getWidth() / 8,
                3 * Gdx.graphics.getHeight() / 4 - hanjaFont.getLineHeight());
        hanjaFont.draw(batch, "所家如知你里公行可使到四至", Gdx.graphics.getWidth() / 8,
                3 * Gdx.graphics.getHeight() / 4 - 2 * hanjaFont.getLineHeight());
        batch.end();
    }
