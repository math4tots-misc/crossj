...
/**
 * Placeholder 'Key' class for when libgdx is not available.
 */
public final class Key {
    ...
    public static String toString(int keycode) {
        if (keycode < 0)
            throw XError.withMessage("keycode cannot be negative, keycode: " + keycode);
        if (keycode > 255)
            throw XError.withMessage("keycode cannot be greater than 255, keycode: " + keycode);
        switch (keycode) {
    ...
    public static int valueOf(String keyname) {
        if (keyNames == null)
            initializeKeyNames();
        return keyNames.getOrElse(keyname, () -> -1);
    }

    /** lazily intialized in {@link Key#valueOf(String)} */
